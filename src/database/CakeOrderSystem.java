...
public class CakeOrderSystem {
	...
	private static void showOrderDetail(Connection conn, String orderId) {
		String orderInfoSQL = "SELECT o.orders_date, c.customer_name AS customer_name " +
							...
							"WHERE o.orders_id = ?";
		String orderItemsSQL = "SELECT ca.cake_id, ca.cake_name AS cake_name, ca.price AS price, " +
							"SUM(oi.quantity) AS total_quantity, SUM(oi.quantity * ca.price) AS total_price " +
							"FROM orderitem oi " +
			...
			try (ResultSet orderInfoRs = orderInfoStmt.executeQuery()) {
				if (orderInfoRs.next()) {
					...
					try (ResultSet itemRs = orderItemsStmt.executeQuery()) {
						int total=0;
						while (itemRs.next()) {
							String cakeName = itemRs.getString("ca.cake_name");
	...
	private static void updateOrder(Connection conn) {
		...
		// 주문 내역 수정 선택
			...
			System.out.print("새로운 케이크 ID를 입력하세요 (변경하지 않는다면 기존 케이크 ID 입력): ");
